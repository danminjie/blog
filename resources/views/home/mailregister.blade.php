  ...
  <header class="header-wrap" id="nav-scroll"> 
    ...
    <div class="login_box">
      ...
      <div class="login">
          ...
          <form method="post" action="{{ url('domailregister') }}" method="post" onSubmit="return confirm();">
            {{ csrf_field() }}
              <input id="user_name" name="user_name" type="text"  placeholder="请输入邮箱">
              <input id="user_pass" name="user_pass" type="password"  placeholder="密码" />
              <input id="repassword" type="password"  placeholder="重复密码" />

              <input id="yanzhengma" value="注册" style="width:100%;" type="submit">
          </form>
          <div>
          	<br>
          	<a href="{{ url('/forget') }}">忘记密码?</a><a style="margin-left: 150px" href="{{ url('/phoneregister') }}">切换到手机注册</a>
          </div>
      </div>
</div>
...
<script>
  $('#user_name').blur(function() {
    ...
    $.get('exists',{'user_name':phone},function(data){
      if(data.status == 0){
          layer.msg('邮箱已被注册',{'time':3000,'icon':5});
          $('#user_name').focus();
      }else{
        return true;         
      }
