  ...
  <header class="header-wrap" id="nav-scroll"> 
    ...
    <div class="login_box">
      ...
      <div class="login" style="margin-top: -40px">
          ...
          <form method="post" action="{{ url('/dophoneforget') }}" onSubmit="return confirm();">
          	{{ csrf_field() }}
              ...
              <input id="user_pass" name="user_pass" type="password"  placeholder="新密码" />
              <input id="user_repass" name="user_repass" type="password"  placeholder="确认新密码" />

              <input value="找回密码" style="width:100%;" type="submit">
          </form>
          <div>
          	<br>
          	<a href="{{ url('/login') }}">去登录页面</a><a style="margin-left: 140px" href="{{ url('/mailforget') }}">切换到邮箱找回</a>
          </div>
      </div>
  </div>
  ...
  <script>
  	...
  	function sendCode(){
        ...
        $('#user_name').focus();
  		...
  		$.get('exists',{'user_name':phone},function(data){
              if(data.status != 0){
                  layer.msg('账号不存在,请检查',{'time':3000,'icon':5});
                  $('#user_name').focus();
                  return false;
              }else{
  				settime();
  	...
  	function confirm()
       ...
       　　return false;
        ...
        else if($("#user_pass").val() != $("#user_repass").val())
       {
